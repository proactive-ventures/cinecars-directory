...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${SITE_URL}/tv-series`, lastModified: new Date(), changeFrequency: "weekly", priority: 0.8 },
  ...
  const carPages: MetadataRoute.Sitemap = cars.map((car) => ({
    ...
    priority: 0.7,
    images: car.image || car.imageUrl ? [car.image?.startsWith("http") ? car.image : `${SITE_URL}${car.image || car.imageUrl || ""}`] : undefined,
  }))
  ...
  const moviePages: MetadataRoute.Sitemap = movies.map((movie) => ({
    ...
    changeFrequency: "weekly",
    priority: 0.6,
    images: movie.image ? [`${SITE_URL}${movie.image}`] : undefined,
  }))

  const tvPages: MetadataRoute.Sitemap = tvSeries.map((series) => ({
    url: `${SITE_URL}/tv-series/${series.slug}`,
    lastModified: new Date(),
    changeFrequency: "weekly",
    priority: 0.6,
    images: series.image ? [`${SITE_URL}${series.image}`] : undefined,
  }))

  const decadePages: MetadataRoute.Sitemap = decades.map((decade) => ({
  ...
  const makePages: MetadataRoute.Sitemap = makeSlugs.map((make) => ({
